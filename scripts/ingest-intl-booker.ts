/**
 * ingest-intl-booker.ts (books arc - list 5/8): International Booker Prize 2016-2026 (novel-era only; pre-2016 honored authors, excluded), winners only; py = English-translation pub year; 2026 winner web-verified
 *
 ...
 * Plan → scripts/intl-booker-plan.json; live → scripts/intl-booker-created-ids.json
...
const WINNERS: Entry[] = [
  { t: "The Vegetarian", a: "Han Kang", tok: "kang", y: 2016, py: 2015 },
  { t: "A Horse Walks into a Bar", a: "David Grossman", tok: "grossman", y: 2017, py: 2016 },
  { t: "Flights", a: "Olga Tokarczuk", tok: "tokarczuk", y: 2018, py: 2017 },
  { t: "Celestial Bodies", a: "Jokha Alharthi", tok: "alharthi", y: 2019, py: 2018 },
  // credited as Lucas Rijneveld on current editions (author's name change post-2021)
  { t: "The Discomfort of Evening", a: "Lucas Rijneveld", tok: "rijneveld", y: 2020, py: 2020 },
  { t: "At Night All Blood Is Black", a: "David Diop", tok: "diop", y: 2021, py: 2020 },
  { t: "Tomb of Sand", a: "Geetanjali Shree", tok: "shree", y: 2022, py: 2021 },
  { t: "Time Shelter", a: "Georgi Gospodinov", tok: "gospodinov", y: 2023, py: 2022 },
  { t: "Kairos", a: "Jenny Erpenbeck", tok: "erpenbeck", y: 2024, py: 2023 },
  { t: "Heart Lamp", a: "Banu Mushtaq", tok: "mushtaq", y: 2025, py: 2025, alt: ["Heart Lamp: Selected Stories"] },
  { t: "Taiwan Travelogue", a: "Yang Shuang-zi", tok: "zi", y: 2026, py: 2024, alt: ["Taiwan Travelogue: A Novel"] },
];
...
async function main() {
  console.log(`International Booker mark+ingest — ${DRY ? "DRY RUN (no writes)" : "LIVE"}\n`);
  console.log(`winners embedded: ${WINNERS.length} (2016-2026, novel era only)\n`);
  ...
  writeFileSync("scripts/intl-booker-plan.json", JSON.stringify({ mark: toMark.map((m) => ({ y: m.e.y, t: m.e.t, itemId: m.itemId })), ingest: plan }, null, 1));
  console.log(`\nPlan → scripts/intl-booker-plan.json`);

  if (DRY) {
    console.log(`\n[DRY RUN] No writes. Live: ${toMark.length} marks + ${plan.length} ingests + ${toMark.length + plan.length} intl_booker Award rows.`);
  ...
  for (const p of plan) {
    ...
    console.log(`  ✓ #${item.id} "${p.entry.t}" (pub ${pyOf(p.entry)}, IB ${p.entry.y})`);
    await sleep(DELAY_MS);
  }

  writeFileSync("scripts/intl-booker-created-ids.json", JSON.stringify({ marked: toMark.map((m) => m.itemId), created }, null, 1));
  console.log(`\n✅ ${mr.count} marked + ${created.length} created (→ scripts/intl-booker-created-ids.json).`);
